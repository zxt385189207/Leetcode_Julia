...
public class Solution {
    public Node CloneGraph(Node node)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (Node n in cur.neighbors)
            {
                // 图中每个值都是唯一的, 查看是否已经遍历过
                if (!dic.ContainsKey(n.val))
                {
                    queue.Enqueue(n);
                    Node temp = new Node(n.val, new List<Node>());
                    dic.Add(n.val, temp);
                }

                dic[cur.val].neighbors.Add(dic[n.val]);
            }
        }
